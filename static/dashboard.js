...
const ROLE_SIDEBAR_MENUS = {
  ...
  Pharmacy: [
    ...
    { id: 'inventory', name: 'Inventory Management', icon: 'fa-boxes-stacked' },
    { id: 'billing', name: 'Billing & Settlements', icon: 'fa-file-invoice-dollar' },
    { id: 'crm', name: 'Patient CRM', icon: 'fa-users-gear' }
  ],
  Admin: [
...
// Fitness tracker initial values
let fitnessState = {
  steps: 8432,
  walkKm: 5.8,
  runKm: 2.1,
    ...
    ROLE_SIDEBAR_MENUS[role].forEach((menu, index) => {
      ...
      item.onclick = (e) => {
        ...
        if (role === 'Patient') {
          ...
          switchPharmacySubPanel(menu.id);
        }
      };
...
async function verifyConsentOtp() {
  ...
  // Verify against dynamic consent OTP
  const isMatch = (activeConsentOTP && otpVal === activeConsentOTP);
...
function downloadPharmacyRx(orderId) {
  // Simulate downloading a PDF prescription
  const content = `Digital Prescription Document for Order ${orderId}\nGenerated by SehatRecover Blockchain Gateway.`;
  const blob = new Blob([content], { type: 'text/plain' });
  const url = window.URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `Prescription_${orderId}.txt`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  window.URL.revokeObjectURL(url);
  addAuditLogLine('info', `Prescription for order ${orderId} downloaded by Pharmacy node.`);
}
...
// ==========================================
// PHARMACY PANEL: NAVIGATION & CHAT
// ==========================================

function switchPharmacySubPanel(panelId) {
  const panels = document.querySelectorAll('.pharmacy-sub-panel');
  panels.forEach(p => p.classList.add('hidden'));
  
  const activePanel = document.getElementById(`pharmacy-panel-${panelId}`);
  if (activePanel) activePanel.classList.remove('hidden');
...
function getCustomMOQ(user) {
  // Custom MOQ logic: Tier based on transactions (rewardPoints)
  // High tier (> 500 pts): 10 boxes MOQ
  // Medium tier (> 200 pts): 25 boxes MOQ
  // Default tier: 50 boxes MOQ
  if (user && user.rewardPoints > 500) {
    return 10;
  } else if (user && user.rewardPoints > 200) {
    return 25;
  }
  return 50;
}

function updateWholesaleMOQLabel() {
  const minMoq = getCustomMOQ(currentUser);
  const qtyInput = document.getElementById('ws-quantity');
  if (qtyInput) {
    qtyInput.min = minMoq;
    qtyInput.placeholder = `Min ${minMoq} boxes (Tier custom)`;
    
    // Update the label helper hint if it exists
    const label = qtyInput.previousElementSibling;
    if (label) {
      label.innerHTML = `Quantity (Boxes - 10x10 packing) <span style="color:#ef4444;">*</span> <span style="color:#10b981; font-weight:bold; font-size:0.7rem; margin-left:8px;">Your custom MOQ: ${minMoq} boxes</span>`;
    }
  }
}
...
function openPharmacyChatModal(orderId) {
  activePharmacyChatOrder = orderId;
  const chatLog = document.getElementById('pharmacy-chat-log');
  chatLog.innerHTML = `<div style="text-align:center; color:#7d9696; font-size:0.8rem; margin-bottom:10px;">Chat started for Order ${orderId}</div>
  <div style="text-align:center; color:#ef4444; font-size:0.75rem; margin-bottom:15px;"><i class="fa-solid fa-user-shield"></i> Privacy Filter Active: Phone numbers and emails are automatically redacted.</div>`;
  document.getElementById('pharmacy-chat-modal').classList.remove('hidden');
}

function closePharmacyChatModal() {
  document.getElementById('pharmacy-chat-modal').classList.add('hidden');
  activePharmacyChatOrder = null;
}

function sendPharmacyChatMessage() {
  const input = document.getElementById('pharmacy-chat-input');
  let text = input.value.trim();
  if (!text) return;
  
  // Privacy Filter: Redact 10-digit numbers and emails
  const phoneRegex = /\b\d{10}\b/g;
  const emailRegex = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g;
  
  const originalText = text;
  text = text.replace(phoneRegex, "[REDACTED NUMBER]").replace(emailRegex, "[REDACTED EMAIL]");
  
  if (originalText !== text) {
    showToast("Privacy Filter Applied", "Sensitive contact information was redacted from your message before sending.", "info");
    addAuditLogLine('warning', `Pharmacy chat privacy filter triggered on order ${activePharmacyChatOrder}. Contact info redacted.`);
  }

  const chatLog = document.getElementById('pharmacy-chat-log');
  chatLog.innerHTML += `
    <div style="margin-bottom:10px; text-align:right;">
      <div style="display:inline-block; background:rgba(0,204,180,0.2); border:1px solid rgba(0,204,180,0.3); padding:8px 12px; border-radius:8px 8px 0 8px; color:#fff; text-align:left; max-width:80%;">
        ${text}
      </div>
      <div style="font-size:0.6rem; color:#7d9696; margin-top:4px;">You (Pharmacist)</div>
    </div>
  `;
...
function initiateIVRVoiceCall(patientName, phoneNumber) {
  ...
  // Show sequence of toasts to simulate the IVR connection
...
async function submitWholesaleOrder(e) {
  ...
  const minMoq = getCustomMOQ(currentUser);
  if (quantity < minMoq) {
    showToast("MOQ Error", `Minimum Order Quantity for your transaction tier is ${minMoq} boxes.`, "error");
    return;
  }
...
// ==========================================
// IoMT DEVICE CONNECT WORKFLOW SIMULATION
// ==========================================
function syncIoMTUIState() {
  const grid = document.getElementById('iomt-telemetry-grid');
  const banner = document.getElementById('iomt-offline-banner');
  const badge = document.getElementById('iomt-status-badge');
  const disconnectBtn = document.getElementById('iomt-disconnect-btn');
  
  if (!grid || !banner || !badge) return;
  
  if (iomtState.connected) {
    grid.classList.remove('offline');
    banner.classList.add('hidden');
    badge.style.backgroundColor = 'rgba(16, 185, 129, 0.1)';
    badge.style.color = '#10b981';
    badge.innerHTML = `<i class="fa-solid fa-circle-dot pulse-dot"></i> Connected: ${iomtState.deviceName}`;
    ...
    const pVal = document.getElementById('iomt-pulse');
    const sVal = document.getElementById('iomt-spo2');
    const tVal = document.getElementById('iomt-temp');
    const bVal = document.getElementById('iomt-bp');
    
    if (pVal) pVal.innerText = "--";
    if (sVal) sVal.innerText = "--";
    if (tVal) tVal.innerText = "--";
    if (bVal) bVal.innerText = "--";
    
    const pStat = document.getElementById('iomt-pulse-status');
    const sStat = document.getElementById('iomt-spo2-status');
    const tStat = document.getElementById('iomt-temp-status');
    const bStat = document.getElementById('iomt-bp-status');
    
    if (pStat) pStat.innerText = "Offline";
    if (sStat) sStat.innerText = "Offline";
    if (tStat) tStat.innerText = "Offline";
    if (bStat) bStat.innerText = "Offline";
  }
}

function openIoMTModal() {
  const modal = document.getElementById('iomt-connect-modal');
  if (modal) modal.classList.add('active');
  nextIoMTStep(1);
}

function closeIoMTModal() {
  const modal = document.getElementById('iomt-connect-modal');
  if (modal) modal.classList.remove('active');
}

let activeIoMTStepIndex = 1;
function nextIoMTStep(stepIndex) {
  activeIoMTStepIndex = stepIndex;
  
  // Hide all step views
  for (let i = 1; i <= 5; i++) {
    const view = document.getElementById(`iomt-step-${i}`);
    const dot = document.getElementById(`dot-step-${i}`);
    if (view) {
      if (i === stepIndex) view.classList.remove('hidden');
...
function selectIoMTCategory(category, element) {
  iomtState.category = category;
  
  // Update UI active card
  const cards = element.parentElement.querySelectorAll('.iomt-choice-card');
  cards.forEach(c => c.classList.remove('active'));
  element.classList.add('active');
}

function selectIoMTProtocol(protocol, element) {
  iomtState.protocol = protocol;
...
let mockDevices = {
  smartwatch: [
    { name: "Apple Watch Ultra 2", rssi: "Excellent (-45dB)" },
    { name: "Galaxy Watch 6 Pro", rssi: "Good (-58dB)" },
    { name: "Garmin Fenix 7", rssi: "Fair (-68dB)" }
  ],
  fitband: [
    { name: "Fitbit Charge 6", rssi: "Excellent (-48dB)" },
    { name: "Whoop Strap 4.0", rssi: "Good (-61dB)" }
  ],
  bpmonitor: [
    { name: "Omron Evolve Smart BP", rssi: "Excellent (-38dB)" }
  ],
  pulseox: [
    { name: "Wellue PulseOx Smart", rssi: "Excellent (-42dB)" }
  ]
};

function simulateBleDiscovery() {
  const radar = document.getElementById('ble-radar-scan');
  ...
  if (!radar || !list) return;
  
  // Show radar, hide list, disable buttons
  radar.classList.remove('hidden');
  list.classList.add('hidden');
  if (retryBtn) retryBtn.disabled = true;
  if (backBtn) backBtn.disabled = true;
  desc.innerText = "Scanning for nearby Bluetooth smart devices...";
  
  setTimeout(() => {
    // Hide radar, show list, enable buttons
    radar.classList.add('hidden');
    list.classList.remove('hidden');
    if (retryBtn) retryBtn.disabled = false;
    if (backBtn) backBtn.disabled = false;
    desc.innerText = "Select a discovered peripheral device below:";
    
    // Inject mock devices based on category
    list.innerHTML = '';
    const devices = mockDevices[iomtState.category] || [];
    
    devices.forEach(dev => {
      const devItem = document.createElement('div');
      devItem.className = 'ble-device-item';
      devItem.onclick = () => {
        iomtState.deviceName = dev.name;
        nextIoMTStep(4);
      };
      
      let icon = "fa-stopwatch";
      if (iomtState.category === 'fitband') icon = "fa-heart-pulse";
      if (iomtState.category === 'bpmonitor') icon = "fa-gauge-high";
      if (iomtState.category === 'pulseox') icon = "fa-wind";
      ...
      devItem.innerHTML = `
        ...
        <span class="ble-signal"><i class="fa-solid fa-signal"></i> ${dev.rssi}</span>
      `;
      list.appendChild(devItem);
    });
  }, 2200); // 2.2 seconds scanning simulation
}

function handlePinFocus(input, index) {
...
function verifyIoMTPinning() {
  let pin = "";
  for (let i = 1; i <= 6; i++) {
    pin += document.getElementById(`pin-${i}`).value.trim();
  }
  
  if (pin === "849201") {
    // Success! Update success page fields
    document.getElementById('paired-device-name').innerText = iomtState.deviceName;
    
    // Simulate battery level
    iomtState.battery = Math.floor(75 + Math.random() * 20);
    document.getElementById('paired-device-battery').innerHTML = `<i class="fa-solid fa-battery-three-quarters"></i> ${iomtState.battery}%`;
    
    nextIoMTStep(5);
  } else {
    showToast("Pairing Failed", "Invalid authentication pairing PIN. Please try again.", "error");
    // Clear PIN inputs
    for (let i = 1; i <= 6; i++) {
...
function completeIoMTConnection() {
  iomtState.connected = true;
  
  // Persist state in sessionStorage so it survives reload
  sessionStorage.setItem('sehatrecover_iomt', JSON.stringify(iomtState));
  
  // Sync dashboard card UI and start telemetry sync
  syncIoMTUIState();
  closeIoMTModal();
  
  showToast("Device Connected", `Successfully linked to ${iomtState.deviceName}. Cryptographic live telemetry stream active.`, "success");
  addAuditLogLine("success", `Secure IoMT linkage established: ${iomtState.deviceName} synchronized successfully.`);
}

function disconnectIoMT(e) {
  if (e) e.preventDefault();
  
  iomtState.connected = false;
  iomtState.deviceName = "";
  sessionStorage.removeItem('sehatrecover_iomt');
  
  syncIoMTUIState();
  
  showToast("Device Disconnected", "Telemetry stream stopped. Sensors offline.", "info");
  addAuditLogLine("info", "Secure IoMT linkage terminated by patient user request.");
}

function checkIoMTConnectionOnLoad() {
  const savedState = sessionStorage.getItem('sehatrecover_iomt');
  if (savedState) {
